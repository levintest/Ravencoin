...
AC_DEFUN([RAVEN_QT_CONFIGURE],[
  ...
  if test "x$use_pkgconfig" = xyes; then
    RAVEN_QT_CHECK([_RAVEN_QT_FIND_LIBS_WITH_PKGCONFIG])
  else
  ...
  if test "x$raven_cv_static_qt" = xyes; then
    ...
    AC_CACHE_CHECK(for Qt < 5.4, raven_cv_need_acc_widget,[
  ...
  AC_MSG_RESULT([$raven_enable_qt (Qt5)])
...
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: raven_cv_qt5=yes|no
AC_DEFUN([_RAVEN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, raven_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
AC_DEFUN([_RAVEN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$raven_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, raven_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
         ...
         if test "x$raven_cv_need_platformsupport" = xyes; then
           if test x$raven_cv_qt58 = xno; then
             ...
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FontDatabaseSupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXFontDatabaseSupport not found)))
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}EventDispatcherSupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXEventDispatcherSupport not found)))
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}ThemeSupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXThemeSupport not found)))
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FbSupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXFbSupport not found)))
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}DeviceDiscoverySupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXDeviceDiscoverySupport not found)))
             RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}AccessibilitySupport],[main],,RAVEN_QT_FAIL(lib$QT_LIB_PREFIXAccessibilitySupport not found)))
...
AC_DEFUN([_RAVEN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  if test x$raven_cv_qt58 = xno; then
    ...
    RAVEN_QT_CHECK(AC_SEARCH_LIBS([png_error] ,[qtlibpng png],,AC_MSG_WARN([libpng not found. Assuming qt has it built-in])))
    RAVEN_QT_CHECK(AC_SEARCH_LIBS([pcre2_match_16], [qtpcre2 libqtpcre2],,AC_MSG_WARN([libqtpcre2 not found. Assuming qt has it built-in])))
  fi
  RAVEN_QT_CHECK(AC_SEARCH_LIBS([hb_ot_tags_from_script] ,[qtharfbuzzng qtharfbuzz harfbuzz],,AC_MSG_WARN([libharfbuzz not found. Assuming qt has it built-in or support is disabled])))
  RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Core]   ,[main],,RAVEN_QT_FAIL(lib${QT_LIB_PREFIX}Core not found)))
  RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Gui]    ,[main],,RAVEN_QT_FAIL(lib${QT_LIB_PREFIX}Gui not found)))
  RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Network],[main],,RAVEN_QT_FAIL(lib${QT_LIB_PREFIX}Network not found)))
  RAVEN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Widgets],[main],,RAVEN_QT_FAIL(lib${QT_LIB_PREFIX}Widgets not found)))
  QT_LIBS="$LIBS"
  LIBS="$TEMP_LIBS"
